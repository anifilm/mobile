  ...
  <v-container flow>
    ...
    <v-row>
      ...
      <v-col class="text-center" cols="8" offset="2" sm="6" offset-sm="3">
        <!-- 양식의 입력이 제출 되면 페이지를 다시 로드 하지 않도록 prevent	사용 -->
        <form v-on:submit.prevent="fnRegisterUser">
          ...
          <v-text-field name="ConfirmPassword" label="비밀번호 확인" type="password" v-model="sConfirmPassword" required v-bind:rules="[fnComparePassword]"></v-text-field>
          <!-- 시간 지연 상태인 경우 버튼은 사라지게 함 -->
          <v-btn type="submit" v-if="!fnGetLoading" color="orange" dark>회원가입</v-btn>
          <!-- 시간지연의 경우 회전 프로그레스 원 표시 -->
          <v-progress-circular v-if="fnGetLoading" indeterminate v-bind:width="7" v-bind:size="70" color="grey lighten-1">
          </v-progress-circular>
          <!-- 오류 메시지가 있을 경우 표시 -->
...
export default {
  ...
  computed: {
    ...
    fnComparePassword() {
      // 비밀번호 길이 우선 검사
      if (this.sPassword.length <= this.sConfirmPassword.length) {
        // 비밀번호 길이가 같은 경우 같은 비밀번호인지 검사
